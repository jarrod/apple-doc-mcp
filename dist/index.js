...
var package_default = {
  ...
  repository: {
    type: "git",
    url: "https://github.com/jarrod/apple-doc-mcp.git"
  },
  bugs: {
    url: "https://github.com/jarrod/apple-doc-mcp/issues"
  },
  homepage: "https://github.com/jarrod/apple-doc-mcp#readme",
  scripts: {
    dev: "bun --watch src/index.ts",
    build: "bun build ./src/index.ts --outdir ./dist --target bun",
    compile: "bun build ./src/index.ts --compile --outfile ./dist/apple-doc-mcp-server",
    lint: "xo src",
    "lint:fix": "xo src --fix",
...
// src/apple-client/cache/memory-cache.ts
class MemoryCache {
  cache = new Map;
  cacheTimeout;
...
class HttpClient {
  ...
  async makeRequest(path) {
    ...
    const cached = this.cache.get(url2);
    if (cached) {
      return cached;
    }
    try {
      ...
      this.cache.set(url2, response.data);
      return response.data;
    } catch (error) {
      console.error(`Error fetching ${url2}:`, error instanceof Error ? error.message : String(error));
...
// src/apple-client/formatters.ts
var extractText = (abstract = []) => abstract?.map((item) => item.text).join("") || "";
var formatPlatforms = (platforms) => {
  if (!platforms || platforms.length === 0) {
    return "All platforms";
  }
  return platforms.map((p) => `${p.name} ${p.introducedAt}${p.beta ? " (Beta)" : ""}`).join(", ");
};

// src/apple-client.ts
class AppleDevDocsClient {
...
// src/server/services/local-symbol-index.ts
...
class LocalSymbolIndex {
  ...
  constructor(client, technologyIdentifier) {
    this.client = client;
    this.cacheDir = join2(__dirname3, "../../../.cache");
  ...
  async buildIndexFromCache() {
    ...
    for (const file of files) {
      const filePath = join2(this.cacheDir, file);
      try {
  ...
  search(query, maxResults = 20) {
    ...
    for (const [id, entry] of this.symbols.entries()) {
      ...
      if (hasWildcards) {
        ...
        if (regex.test(entry.title.toLowerCase()) || regex.test(entry.path.toLowerCase()) || entry.tokens.some((token) => regex.test(token))) {
          score = 100;
        }
      } else {
    ...
    return results.sort((a, b) => b.score - a.score).slice(0, maxResults).map((result) => result.entry);
  }
  getSymbolCount() {
...
class ServerState {
  ...
  getLocalSymbolIndex(client) {
    if (!this.localSymbolIndex) {
      const technologyIdentifier = this.activeTechnology?.identifier?.replace("doc://com.apple.documentation/", "")?.replace(/^documentation\//, "");
...
var header = (level, text) => `${"#".repeat(Math.max(1, level))} ${text}`;
var bold = (label, value) => `**${label}:** ${value}`;
var trimWithEllipsis = (text, maxLength) => {
...
// src/server/handlers/discover.ts
var formatPagination = (query, currentPage, totalPages) => {
  if (totalPages <= 1) {
    return [];
  }
...
var buildDiscoverHandler = ({ client, state }) => async (args) => {
  ...
  if (query) {
    const lowerQuery = query.toLowerCase();
    filtered = frameworks.filter((tech) => tech.title.toLowerCase().includes(lowerQuery) || client.extractText(tech.abstract).toLowerCase().includes(lowerQuery));
  }
...
// src/server/handlers/choose-technology.ts
var fuzzyScore = (a, b) => {
  if (!a || !b) {
...
var ensureFramework = (technology) => {
...
var buildChooseTechnologyHandler = ({ client, state }) => async (args) => {
  ...
  if (!chosen && normalizedName) {
    const scored = candidates.map((tech) => ({ tech, score: fuzzyScore(tech.title, name) })).sort((a, b) => a.score - b.score);
  ...
  if (!chosen) {
    ...
    const suggestions = candidates.filter((tech) => tech.title?.toLowerCase().includes(searchTerm)).slice(0, 5).map((tech) => `\u2022 ${tech.title} \u2014 \`choose_technology "${tech.title}"\``);
    const lines2 = [
      header(1, "\u274C Technology Not Found"),
      `Could not resolve "${name ?? identifier ?? "unknown"}".`,
      "",
      header(2, "Suggestions"),
      ...suggestions.length > 0 ? suggestions : ['\u2022 Use `discover_technologies { "query": "keyword" }` to find candidates']
    ];
    return {
      content: [{ text: lines2.join(`
`), type: "text" }]
    };
  }
...
// src/server/handlers/no-technology.ts
var buildNoTechnologyMessage = ({ client, state }) => async () => {
  const lastDiscovery = state.getLastDiscovery();
  let availableTechnologies = [];
  try {
    const technologies = await client.getTechnologies();
    availableTechnologies = Object.values(technologies).filter((tech) => tech.title && tech.kind === "symbol" && tech.role === "collection").slice(0, 8).map((t) => t.title);
  } catch (error) {
...
// src/server/handlers/current-technology.ts
var buildCurrentTechnologyHandler = (context) => {
...
// src/server/services/framework-loader.ts
var loadActiveFrameworkData = async ({ client, state }) => {
...
// src/server/handlers/get-documentation.ts
var formatIdentifiers = (identifiers, references, client) => {
  const content = [];
...
var formatTopicSections = (data, client) => {
  const content = [];
...
var buildGetDocumentationHandler = (context) => {
...
// src/server/handlers/search-symbols.ts
var buildSearchSymbolsHandler = (context) => {
  const { client, state } = context;
  const noTechnology = buildNoTechnologyMessage(context);
  return async (args) => {
    const activeTechnology = state.getActiveTechnology();
    ...
    const { query, maxResults = 20, platform, symbolType: symbolType2 } = args;
    ...
    if (symbolResults.length === 0 && techLocalIndex.getSymbolCount() < 50) {
      ...
      const frameworkResults = await client.searchFramework(activeTechnology.title, query, { maxResults: maxResults * 2, platform, symbolType: symbolType2 });
    ...
    if (symbolType2) {
      const typeLower = symbolType2.toLowerCase();
...
// src/server/handlers/version.ts
var buildVersionHandler = () => async () => ({
  content: [
    {
      type: "text",
      text: `Apple Doc MCP Server Version Information:

\uD83D\uDCE6 Package Version: ${package_default.version}
\uD83C\uDFF7\uFE0F  Server Name: ${package_default.name}
\uD83D\uDCDD Description: ${package_default.description}
\uD83D\uDC64 Author: ${package_default.author}
\uD83D\uDD17 Repository: ${package_default.repository?.url ?? "N/A"}
...
// src/server/tools.ts
var registerTools = (server, context) => {
